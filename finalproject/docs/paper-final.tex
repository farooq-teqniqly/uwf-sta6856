...
The following PySpark script was used to preprocess over 150 million records from the NYC FHV dataset and compute the average daily trip durations.

First, the parquet file is loaded (Listing ~\ref{lst:pyspark_load}).

\begin{lstlisting}[style=pystyle, caption={Load FHV parquet file code}, label={lst:pyspark_load}]
!pip install pyspark
...
\begin{lstlisting}[style=pystyle, caption={Adding derived columns code}, label={lst:pyspark_derived_cols}]
